...
/** ----- Types that match your server /posts shape ----- */
type Comment = {
  id: string;
  text: string;
  username: string;
...
type Post = {
  ...
  comments: Comment[];
  aiEnabled?: boolean;        // client-only
};

/** Event shape emitted by the webhook socket */
type NewCommentEvt = {
  postId?: string;
    ...
    socket.on('ig:new_comment', (evt: NewCommentEvt) => {
      ...
      setPosts(prev =>
        prev.map(p => {
          ...
          const already = p.comments?.some(c => c.id === comment.id);
          let comments: Comment[];

          if (verb === 'remove') {
            comments = (p.comments || []).filter(c => c.id !== comment.id);
          } else {
            comments = already ? (p.comments || []) : [comment, ...(p.comments || [])];
          }

          const delta =
            verb === 'remove' ? -1 : already ? 0 : 1;

          return {
            ...p,
            comments,
            commentsCount: Math.max(0, (p.commentsCount ?? comments.length) + delta),
          };
        })
      );
    });
            ...
            <div className="relative">
              ...
              <button
                ...
                className={`absolute top-3 right-3 p-2 rounded-full ${
                  post.aiEnabled ? 'bg-green-500 text-white' : 'bg-gray-500 text-white'
